...
class Base extends Model
{
    ...
    public function getOne($pk = 0, $refresh = 0){
        $id = $this->getId($pk);
        $cache_key = config('database.hostname') . config('database.database') . $this->getTrueTable($pk) . '::' . $id;
    ...
    public function delOne($pk=0){
        ...
        if($this->getBuilder($pk)->delete($id)){
            cache($this->getTrueTable($pk) . '::' . $id, null);
            return true;
        }
        return false;
    }
    ...
    public function total($where = [], $refresh = 0){
        $cache_key = config('database.hostname') . config('database.database') . $this->getTrueTable($where) . '::total' . ':' . serialize($where);
        $refresh && cache($cache_key, null);
    ...
    private function _where(&$self, $where=[]){
        ...
        if($where){
            foreach($where as $k => $w){
                if(! is_int($k)){
                    //过滤不存在的字段
                    $where = array_filter($where, function ($key) use ($fields) {
                        # 过滤不需要的键值
                ...
                list($_k, $_v) = $w;
                in_array($_k, $fields) && $self->where($w);
                //$self->where($w);
            }
        }
        return $self;
     ...
     * @Author: Doogie <461960962@qq.com>
     */
    public function getOneByMap($where=[], $refresh = 0,$field = true){
        $cache_key = config('database.hostname') . config('database.database') . $this->getTrueTable($where) . '::getonebymap' . ':' . serialize($where);
        $refresh && cache($cache_key, null);
        $selector = $this->getBuilder($where)->field($field);
        $this->_where($selector, $where);
        if($this->isCache){
    ...
    public function getBuilder($query = []){
        if($this->rule && $this->key){
            return $this->db()->partition($query, $this->key, $this->rule);
        }else{
            return $this->db();
        }
    }
    ...
    public function getTrueTable($query = []){
        if($this->key && !empty($query[$this->key])){
            return $this->db()->getPartitionTableName([$this->key => $query[$this->key]], $this->key, $this->rule);
        }else{
     ...
     * @author: Doogie<461960962@qq.com>
     */
    public function getAll($where=[], $order=[],$filed = true,$refresh = 0){
        $cache_key = config('database.hostname') . config('database.database') . $this->getTrueTable($where) . '::getall' . ':' . serialize($where) . ':' . serialize($order);
        $refresh && cache($cache_key, null);
        $selector = $this->getBuilder($where)->field($filed);
        $this->_where($selector, $where);
        if($this->isCache){
            $selector->cache($cache_key, $this->expire);
        }
